...
void MyTunes::executeAddTrack(Command cmd){
	...
	Track track (
	  //id, songs.findByID(stoi(cmd.getToken(SONG_ID)))
		id,
		stoi(cmd.getToken(RECORDING_ID)),
		stoi(cmd.getToken(SONG_ID)),
		stoi(cmd.getToken(TRACK_NUMBER))
	);
...
void MyTunes::executeAddPlaylist(Command cmd){
	...
	User user;
	if(!users.findByUserID(cmd.getToken(USERID), user)) return;
	Playlist playlist(cmd.getToken(PLAYLIST_NAME));
...
void MyTunes::executeAddPlaylistTrack(Command cmd){
	...
	if(!users.findByUserID(cmd.getToken(USERID), user)) return;
	Playlist  playlist;
	if(user.findPlaylist(cmd.getToken(PLAYLIST_NAME), playlist)) return;
	Track track;
		
	if(!tracks.findByID(stoi(cmd.getToken(TRACK_ID)), track)) return;
...
void MyTunes::executeDeleteRecording(Command cmd){
	...
	if(!cmd.isValidIndex(cmd.getToken(ID))) return;
	Recording recording;
	if(!recordings.findByID(stoi(cmd.getToken(ID)), recording)) return;
...
void MyTunes::executeDeleteUser(Command cmd){
	...
	User user;
	if(!users.findByUserID(cmd.getToken(USER_ID), user)) return;
	users.remove(user);
...
void MyTunes::executeDeleteUserPlaylist(Command cmd){
	...
	if(!users.findByUserID(user_id, user)) return;
	
	Playlist playlist;
	
	if (!user.findPlaylist(playlistName, playlist)) {
		return;
	}
...
void MyTunes::executeDeleteUserPlaylistTrack(Command cmd){
  ...
  User user;
	...
	if(!user.findPlaylist(playlistName, playlist)) return;
	Track  track;
	
	if(!tracks.findByID(trackIndex, track)) return;
...
void MyTunes::executeDeleteTrack(Command cmd){
	...
	users.getCollection(theUsers);
	for(vector<User>::iterator itr = theUsers.begin(); itr != theUsers.end(); itr++){
		User user = *itr;
		user.removeTrack(track);

	}
	vector<Recording> theRecordings;
	recordings.getCollection(theRecordings);
